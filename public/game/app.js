...
function skill(c, event) {
  ...
  let ongoing = (c.ongoing || [])
    .map(
      (x, i) =>
        `<div class="skill-section"><b>在場效果${c.ongoing.length > 1 ? i + 1 : ""}</b><br>條件：${esc(x.condition || "—")}<br>效果：${esc(x.effect || "—")}</div>`,
    )
    .join("");
  return `<div class="skill-section"><b>${c.faction}命格 · 事件</b><br>${esc(c.event || "無")}</div><div class="skill-section"><b>關鍵字</b><br>${kw}</div>${ongoing}<div class="skill-section"><b>進場</b><br>條件：${esc(c.entry.costs.join("／") || "—")}<br>效果：${esc(c.entry.effect || "—")}${c.entry.extraEffect ? `<br>追加：${esc(c.entry.extraCondition)} → ${esc(c.entry.extraEffect)}` : ""}</div><div class="skill-section"><b>啟動</b><br>條件：${esc(c.activate.cost || "—")}<br>效果：${esc(c.activate.effect || "—")}${c.activate.extraEffect ? `<br>追加：${esc(c.activate.extraCondition)} → ${esc(c.activate.extraEffect)}` : ""}</div>`;
}
function preview(el) {
...
function choosePlayerTarget(title, text, done) {
  ...
  let buttons = state.players
    .map(
      (p, i) =>
        `<button data-player-target="${i}" class="${i === owner ? "primary" : ""}">${i === owner ? "自己" : `玩家${i + 1}`}：${esc(p.name)}（黑 ${p.black}／白 ${p.white}）</button>`,
    )
    .join("");
  for (let i = state.players.length; i < 4; i++)
    buttons += `<button disabled>玩家${i + 1}（四人模式開放後可選）</button>`;
  $("#actions").innerHTML =
    `<span class="mode-note">${esc(title)}：${esc(text)}</span><div class="player-targets">${buttons}</div>`;
  $$("[data-player-target]").forEach(
    (b) =>
      (b.onclick = () => {
...
function chooseExhaustTargets(spec, title, confirm, cancel) {
  let owners = spec.both ? [state.active, 1 - state.active] : [state.active],
    pool = [];
...
function applyEffect(s, c, after) {
  ...
  if (ops.length) {
    ...
    warn(
      `${c.name}：選擇目標`,
      `${fixed}\n\n此效果尚需手動完成；按確定後會繼續結算後續效果。`,
      after,
    );
  else if (!xs && !state.pendingBarrier) after?.();
}
...
$("#cardPreview").onclick = hidePreview;
