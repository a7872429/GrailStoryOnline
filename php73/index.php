...
(function(){
  ...
  function lobby(msg){app.innerHTML='<p class="eyebrow">LEGEND CHRONICLE</p><h1>傳說紀行</h1><p>選擇遊玩方式</p><label>你的名稱<input id="name" value="玩家" maxlength="12"></label><button id="create2" class="primary">建立雙人房間</button><button id="create4" class="primary">建立四人隊伍房間</button><div class="join"><input id="code" placeholder="輸入六位房號" maxlength="6"><button id="join">加入房間</button></div><a href="game/index.html">同一台電腦遊玩</a><p class="message">'+esc(msg||'')+'</p>';document.getElementById('create2').onclick=function(){create(2)};document.getElementById('create4').onclick=function(){create(4)};document.getElementById('join').onclick=join;document.getElementById('code').oninput=function(){this.value=this.value.toUpperCase()}}
  ...
  function openRoom(x){room=x;drawRoom();clearInterval(timer);timer=setInterval(poll,1500)}
   function seatName(){return room.maxPlayers===4?['紅方一號','藍方一號','紅方二號','藍方二號'][room.seat]:'玩家'+(room.seat+1)}
    function drawRoom(){var ready=room.players===room.maxPlayers;app.innerHTML='<p class="eyebrow">線上對戰房間</p><div class="room-code">'+esc(room.code)+'</div><p>你是'+seatName()+'｜'+room.players+'/'+room.maxPlayers+' 人已連線</p>'+(room.maxPlayers===4?'<p class="team-order">隊伍與順序：紅一 → 藍一 → 紅二 → 藍二</p>':'')+'<div class="status"><i class="'+(ready?'on':'')+'"></i>'+(ready?'所有玩家已就緒':'等待玩家加入（尚缺 '+(room.maxPlayers-room.players)+' 人）')+'</div><p class="timer-rule">60 秒提醒｜120 秒系統代打<br>單回合 5 分鐘或累計 5 次直接判負</p>'+(ready?'<a class="primary" href="game/index.html?room='+encodeURIComponent(room.code)+'&seat='+room.seat+'&token='+encodeURIComponent(room.token)+'">進入對戰桌</a>':'')+'<button id="copy">複製房號</button>';document.getElementById('copy').onclick=function(){if(navigator.clipboard)navigator.clipboard.writeText(room.code)}}
